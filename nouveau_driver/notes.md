...
- 0x002500: 写入 1，启用 FIFO 缓存，表示初始化完成

`nv04_fifo_intr` - FIFO 中断处理函数
- 获取设备状态和中断信息
  - 0x002140: 读取中断开关状态
  - 0x002100: 读取中断状态
  - 0x002500: 读取 FIFO 缓存状态，记作 `reassign`
  - 0x002500: 写入 0，暂时禁用 FIFO 缓存
  - 0x003204: 读取 FIFO 推送器缓存状态寄存器
  - 0x003270: 读取 FIFO GET 指针，指示 FIFO 缓存中下一个要取出的指令地址
- 分析中断状态寄存器
  - 如果有信号量相关的中断状态
    - 0x002100: 写入中断状态，清楚信号量中断
    - 0x00326c: 读取信号量状态/控制寄存器，获取当前信号量状态，记作 `sem`
    - 0x00326c: 写回 `sem | 0x1`
    - 0x003270: 更新 FIFO GET 指针
    - 0x003250: 写入 1，触发 FIFO 缓存拉取操作
- 清除已经处理的中断标志
- 恢复 FIFO 缓存状态
  - 0x002500: 写入 `reassign`，恢复 FIFO 缓存状态

`nv04_fifo_pause` - 暂停 FIFO 队列
- 0x002500: 禁用 FIFO 缓存
- 0x003250: 停止数据拉取操作
- 0x003250: 检查拉取器是否正在忙于处理哈希/句柄翻译
- 0x003250: 检查拉取器是否因拉取失败处于错误状态
- 0x002100: 清除硬件中的 FIFO 缓存错误中断
- 0x003258: 清空 FIFO 缓存中计算的实例哈希/对象查找结果

`nv04_fifo_start` - 启动 FIFO 队列
- 0x003250: 启动数据拉取操作
- 0x002500: 启用 FIFO 缓存

PGRAPH:

`nv40_gr_init` - 初始化图形处理器
- 初始化图形处理器
  - 0x40032c: 置 0，清空上下文寄存器
  - 0x400100: 清除所有待处理的图形中断标志
  - 0x40013c: 启用所有图形中断
  - 0x400080: 清除 debug 状态。
  - 0x400080: 清除 debug 状态并禁用某些 debug 功能。
  - 0x400084: 写入 0x401287c0，设置 PGRAPH Debug 1 寄存器，启用或配置一些图形管道的内部调试/控制位。
  - 0x40008c: 写入 0xe0de8055，设置 PGRAPH Debug 3 寄存器，配置额外的图形管道控制或调试设置。
  - 0x40008c: 写入 0x00008000，设置 PGRAPH Debug 4 寄存器
  - 0x400610: 写入 0x00be3c5f，配置或清除像素限制违规状态
  - 0x400144: 写入 0x10010100，配置图形上下文的控制参数
  - 0x400714: 写入 0xFFFFFFFF，清除图形状态寄存器
  - 0x001540: 读取一个与 FIFO/PRAMIN 或其他内部引擎相关的值，用于确定某种配置位。
  ```c
  ...
  ```
  - 0x400820: 写入 0x83280eff，设置内部管道控制参数1。
  - 0x400824: 写入 0x000000a0，设置内部管道控制参数2。
  - 0x4009b8: 写入 0x0078e366，配置硬件功能或状态1。
  - 0x4009bc: 写入 0x0000014c，配置硬件功能或状态2。
  - 0x400b38: 写入 0x2ffff800，设置 PGRAPH Config/Stall 相关的寄存器。
  - 0x400b3c: 写入 0x00006000，设置 PGRAPH Config/Stall 相关的寄存器。
  - ...
